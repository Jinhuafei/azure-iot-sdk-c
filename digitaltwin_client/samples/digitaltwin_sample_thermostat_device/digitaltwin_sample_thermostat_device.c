...
#include <../digitaltwin_sample_diagnostics/digitaltwin_sample_diagnostics.h>
#include <../digitaltwin_sample_temperature_sensor/digitaltwin_sample_temperature_sensor.h>

// TODO: Fill in DIGITALTWIN_SAMPLE_ROOT_INTERFACE_ID
#define DIGITALTWIN_SAMPLE_ROOT_INTERFACE_ID "dtmi:com:examples:Thermostat;1"
...
#define DIGITALTWIN_SAMPLE_TEMPERATURE_SENSOR_INDEX 2
#define DIGITALTWIN_SAMPLE_DIAGNOSTICS_INDEX 3
...
int main(int argc, char *argv[])
{
    ...
    // Invoke to create Temperature Sensor interface - implemented in a separate library - to create DIGITALTWIN_INTERFACE_CLIENT_HANDLE.
    else if ((interfaceClientHandles[DIGITALTWIN_SAMPLE_TEMPERATURE_SENSOR_INDEX] = DigitalTwinSampleTemperatureSensor_CreateInterface()) == NULL)
    {
        LogError("DigitalTwinSampleTemperatureSensor_CreateInterface failed");
    }
    else if ((interfaceClientHandles[DIGITALTWIN_SAMPLE_DIAGNOSTICS_INDEX] = DigitalTwinSampleDiagnostics_CreateInterface()) == NULL)
    {
        LogError("DigitalTwinSampleDiagnostics_CreateInterface failed");
    }
    ...
    else
    {
        ...
        do
        {
            ...
            {
                (void)DigitalTwinSampleTemperatureSensor_SendTelemetryMessagesAsync(interfaceClientHandles[DIGITALTWIN_SAMPLE_TEMPERATURE_SENSOR_INDEX]);

                (void)DigitalTwinSampleDiagnostics_SendTelemetryMessagesAsync(interfaceClientHandles[DIGITALTWIN_SAMPLE_DIAGNOSTICS_INDEX]);
            }
    ...
    if (interfaceClientHandles[DIGITALTWIN_SAMPLE_TEMPERATURE_SENSOR_INDEX] != NULL)
    {
        DigitalTwinSampleTemperatureSensor_Close(interfaceClientHandles[DIGITALTWIN_SAMPLE_TEMPERATURE_SENSOR_INDEX]);
    }

    if (interfaceClientHandles[DIGITALTWIN_SAMPLE_DIAGNOSTICS_INDEX] != NULL)
    {
        DigitalTwinSampleDiagnostics_Close(interfaceClientHandles[DIGITALTWIN_SAMPLE_DIAGNOSTICS_INDEX]);
    }
